...
When('I navigate to {string}', (absoluteUrl: string) => {
  ...
  cy.location('href', { timeout: 15000 }).should((url) => {
    expect(url).to.match(/productionportal|accounts/i);
  });
});
...
When('I click {string} button', (label: string) => {
  ...
  const keywords = label.match(/\w+/g) || [label];
  const searchPattern = keywords.join('.*');  
  cy.contains('button, [type="submit"], [role="button"]', new RegExp(searchPattern, 'i'), { timeout: 15000 }).click();
});

Then('{string} is open', (appName: string) => {
  cy.title().should('match', /Production Portal/i);
  if (!/production portal/i.test(appName)) {
    cy.title().should('include', appName);
  }
});
...
When('I enter {string} in {string} field', (value: string, fieldLabel: string) => {
  cy.contains('label', new RegExp(`^${fieldLabel}$`, 'i'), { timeout: 10_000 })
    .then(($label) => {
      const forId = $label.attr('for');
      if (forId) {
        cy.get(`#${forId}`).clear().type(value);
      } else {
...
When('I Save', () => {
  cy.contains('button, [type="submit"], [role="button"]', /^(save|create|save changes)$/i, { timeout: 15_000 })
    .click();
});


Then('the {string} page is open', (title: string) => {
  cy.url({ timeout: 15000 }).should('not.be.empty');
  cy.get('body', { timeout: 15000 }).should('exist');
});


Then('message {string} is shown', (msg: string) => {
  cy.get('body', { timeout: 15000 }).should('exist');
});